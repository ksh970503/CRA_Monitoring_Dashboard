...
export const IssuesPage: React.FC = () => {
  ...
  const filteredIssues = useMemo(() => {
    return issues.filter((issue) => {
      // Study filter
      if (selectedStudyFilter && issue.study_id !== selectedStudyFilter) {
        return false;
      }
      // Tab filter
  ...
  const getStatusBadge = (st: IssueStatus) => {
    switch (st) {
      case '진행중':
        return <span className="px-2.5 py-1 rounded-xl bg-blue-950/60 text-blue-300 border border-blue-800 text-xs font-bold flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> 진행중</span>;
      case '보류':
        return <span className="px-2.5 py-1 rounded-xl bg-amber-950/60 text-amber-300 border border-amber-800 text-xs font-bold flex items-center gap-1"><Hourglass className="w-3.5 h-3.5" /> 보류</span>;
      case '해결':
        return <span className="px-2.5 py-1 rounded-xl bg-emerald-950/60 text-emerald-300 border border-emerald-800 text-xs font-bold flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> 해결</span>;
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-4 rounded-3xl shadow-xl">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            <span>Waiting for... ({waitingCount})</span>
          </button>
          <button
          ...
          filteredIssues.map((issue) => {
            ...
            return (
              ...
              >
                ...
                <div>
                  <h3 className="text-base font-bold text-white hover:text-blue-300 transition">
                    {issue.title}
                  </h3>
                  {issue.description && (
